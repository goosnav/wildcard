...
**What's not done yet:** see the Phase table below, the REQ → CMP → test matrix further down, and the OQ status section near the bottom. The pre-public-launch gap is mainly P2 polish (home grid long-press + drag + folders, edit-with-AI **version history / revert**), CMP-12 (a conservative input-safety pre-filter now refuses clearly-harmful prompts before any token spend — `server/src/safety.ts` + `eval/disallowed.jsonl`; the model-based moderation pass and the output-side check are still pending), the Privacy Policy / ToS counsel sign-off, and bringing the eval corpus from 134 toward the ≥200 target. (Editable source view, edit-with-AI + version history, account delete/export, manage-subscription, home-grid drag/reorder + rename/duplicate, `/v1/manifest`, `/v1/reports`, input+output safety screens, and in-app legal pages are now shipped.) **Account setup for all external services is documented in [`SETUP.md`](SETUP.md).**
...
- **Corpus size:** 134 / ≥200 prompts (`eval/corpus.jsonl`) across 15 categories — calculator, timer, generator, tracker, notes, list, utility, converter, data, game, health, finance, education, kids, productivity — including multi-feature, stateful, and edge-case prompts that exercise the repair loop. Doc 06 §6.2 calls for ≥200; see ROADMAP.md for the growth plan.
